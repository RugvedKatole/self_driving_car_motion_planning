...
def MPC(z_initial,U_prev,Q,R,S,params,z_r = None):
    """MPC solver"""
    ...
    ymin = np.array([params.wl/2]*N)
    ymax = np.array([params.wl/2 +params.w]*N)
    ...
    # plt.plot(ymin)
    # plt.plot(y)
    # plt.legend(["ref","ymax","ymin","gen"])
    # plt.show()

    return np.array([v,y]) , np.array([v_dash, y_dash])

# z_des = get_ref(0)
# z,u = MPC(z_initial=np.array([[20/3.6],[params.YE0]]),U_prev=np.array([[0],[0]]),z_r=z_des,Q=params.Q,R=params.R ,S=params.S,params=params)
